// ==UserScript==
// @name         SPRNDS - Reenviar v13.4.6
// @namespace    http://tampermonkey.net/
// @version      13.4.6
// @description  Auto-tuning inteligente + busca por status configurável (PENDING/ERROR) + Otimizações
...
(function() {
    ...
    const CONFIG = {
        ...
        dataFim: hojeStr, // ✨ CORREÇÃO: Data dinâmica
    ...
    class CheckpointManager {
        constructor() {
            ...
            this.idsSet = new Set(this.checkpoint.idsSucesso); // ✨ CORREÇÃO 3: Pesquisa O(1)
        }
        ...
        iniciarExecucao() {
            ...
            this.execucaoAtualIdsSet = new Set(); // ✨ O(1) local
    ...
    async function buscarTodasPaginas() {
        ...
        // ✨ CORREÇÃO 3: Map previne duplicatas de registros que aparecem em múltiplos resultados
        ...
        for (const status of statusParaBuscar) {
            ...
            while (page < CONFIG.limiteMaximoPaginas) {
                ...
                // Adiciona ao Map (sobrescreve se ID duplicado, garantindo unicidade)
    ...
    async function iniciarReenvioAPI() {
        ...
        criarModal();
        console.log('🚀 Iniciando reenvio via API Direct v13.4.6...');
    ...
    function exportarCSV() {
        ...
        const objectUrl = URL.createObjectURL(blob); // ✨ CORREÇÃO: Limpar a referência de memória
        
        const link = document.createElement('a');
        link.href = objectUrl;
        link.download = `reenvio_api_v13.4.6_${new Date().toISOString().split('T')[0]}.csv`;
        link.id = 'exportarCSVBtn';
        link.click();

        setTimeout(() => URL.revokeObjectURL(objectUrl), 200); // Libera da memória
    ...
    function inicializar() {
        console.log('');
        console.log('═══════════════════════════════════════════════════════════');
        console.log('🚀 SPRNDS - API Direct v13.4.6');
        console.log('═══════════════════════════════════════════════════════════');
        console.log('✨ NOVO NA v13.4.6:');
        console.log('  • Otimização drástica do Checkpoint (busca em O(1) com Set)');
        console.log('  • Prevenção de duplicatas de IDs na busca por múltiplos status');
        console.log('  • Liberação de memória corrigida na exportação de CSV');
        console.log('═══════════════════════════════════════════════════════════');
        console.log('');
